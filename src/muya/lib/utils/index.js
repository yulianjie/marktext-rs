...
/**
 * Browser-safe replacement for `path.resolve(base, rel)`. Handles both POSIX
 * and Windows-style absolute paths, joins relative segments, and collapses
 * `.` / `..` segments. Returned path uses forward slashes; Chromium normalises
 * them in `file://` URLs.
 */
function resolveRelativePath(base, rel) {
  if (!rel) return base || ''
  const isWinAbs = /^[a-zA-Z]:[\\/]/.test(rel)
  const isPosixAbs = rel.startsWith('/') || rel.startsWith('\\')
  if (isWinAbs || isPosixAbs) {
    return rel.replace(/\\/g, '/')
  }
  const normalisedBase = (base || '').replace(/\\/g, '/')
  const baseSegments = normalisedBase.split('/').filter(Boolean)
  // If base looks like a file (has a non-empty extension), drop the trailing
  // segment so we resolve relative to its directory.
  if (baseSegments.length && /\.[^./]+$/.test(baseSegments[baseSegments.length - 1])) {
    baseSegments.pop()
  }
  const relSegments = rel.replace(/\\/g, '/').split('/').filter(Boolean)
  const out = baseSegments.slice()
  for (const seg of relSegments) {
    if (seg === '..') out.pop()
    else if (seg !== '.') out.push(seg)
  }
  const prefix = /^[a-zA-Z]:$/.test(out[0] || '') ? '' : '/'
